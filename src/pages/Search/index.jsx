...
import { getSearchItems, getSingleSearch } from "../../api/search";
...
function Search() {
  ...
  const [isLoadingSingleSearch, setIsLoadingSingleSearch] = useState(false);
  ...
  const handleFetchSingleSearch = (id) => {
    setIsLoadingSingleSearch(true);
    getSingleSearch(id)
      .then((data) => {
        setSingleSearch(data);
        setIsLoadingSingleSearch(false);
      })
      .catch((error) => {
        console.log(error);
        setIsLoadingSingleSearch(true);
  ...
  return (
    ...
    >
      {count === 0 ? (
        ...
        records.map((record) => (
          <div
            onClick={() => {
              handleOpen();
              handleFetchSingleSearch(record.id);
            }}
      ...
      <Modal
        ...
        sx={{ overflow: "scroll" }}
      >
        {isLoadingSingleSearch ? (
          <CircularProgress size={200} />
        ) : (
          <Box sx={BoxStyle}>
            <div className={`slide-container ${Style["slide-container"]}`}>
              <Slide>
                {singleSearch.images?.map((slideImage, index) => (
                  <div key={index}>
                    <div
                      style={{
                        ...divStyle,
            ...
            <div className={Style["modal__first"]}>
              <h2 className={Style["modal__title"]}>{singleSearch.name}</h2>
              <span className={Style["modal__label"]}>
                <span className={Style["modal__label__dot"]}></span>
                {singleSearch.asset_tag_name}
              </span>
            </div>
            <p className={Style["modal__para"]}>{singleSearch.description}</p>
