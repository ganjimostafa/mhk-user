      ...
      {count === 0 ? (
        ...
        records.map((record) => (
          <div
            onClick={() => {
              handleOpen();
              handleFetchSingleProduct(record.id);
            }}
      ...
      >
        ...
        <Box sx={BoxStyle}>
          <div className={`slide-container ${Style["slide-container"]}`}>
            <Slide>
              {singleProduct.images?.map((slideImage, index) => (
                <div key={index}>
                  <div
                    style={{
                      ...divStyle,
          ...
          <div className={Style["modal__first"]}>
            <h2 className={Style["modal__title"]}>{singleProduct.name}</h2>
            <span className={Style["modal__label"]}>
              <span className={Style["modal__label__dot"]}></span>
              {singleProduct.asset_tag_name}
            </span>
          </div>
          <p className={Style["modal__para"]}>{singleProduct.description}</p>
